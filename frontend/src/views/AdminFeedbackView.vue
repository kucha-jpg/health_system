  ...
  <div class="feedback-admin-page fade-in-page">
      ...
      <el-table-column label="反馈内容" min-width="240">
        <template #default="scope">
          <div class="cell-with-action">
            <span class="cell-text">{{ scope.row.content }}</span>
            <el-button link type="primary" @click="openDetail(scope.row)">查看</el-button>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="处理回复" min-width="220">
        <template #default="scope">
    ...
    <el-dialog v-model="detailVisible" title="反馈详情" width="560px">
      <el-form :model="detailForm" label-width="90px">
        ...
        <el-form-item label="反馈内容">
          <el-input v-model="detailForm.content" type="textarea" :rows="4" disabled />
        </el-form-item>
        <el-form-item label="回复内容">
          <el-input v-model="detailForm.replyContent" type="textarea" :rows="4" disabled />
        </el-form-item>
      </el-form>
      <template #footer>
...
const detailForm = reactive({ senderUsername: '', content: '', replyContent: '' })
let timer = null
let trendChart = null
...
const openDetail = (row) => {
  Object.assign(detailForm, {
    senderUsername: row.senderUsername || '-',
    content: row.content || '-',
    replyContent: row.replyContent || '暂无回复'
  })
