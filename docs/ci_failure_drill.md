...
- 在 `scripts/api_test.sh` 中临时把 `CASE-10` 断言期望改错（例如把 `"True"` 改成 `"False"`），制造 `API error-code assertions gate` 步骤失败。
- 推送后观察失败步骤与日志，再回滚该临时改动并复验通过。
- 该方式可用于演练“全场景回归脚本失败”的定位路径。
...
- 只看 `api_assert.sh` 日志：当前门禁先执行 `api_test.sh`，应先确认场景回归是否通过。
